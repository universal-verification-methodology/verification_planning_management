...
## 4. First Time? (Progressive Path)

If you are new to the course, follow this path so the scripts guide you step by step:

1. **Copy templates to workspace**  
   Copy the files from `module1/templates/` into `module1/` (e.g. `VERIFICATION_PLAN.md`, `REQUIREMENTS_MATRIX.md`, `CHECKLIST.md`). Your workspace is the module root; the script checks these files.

2. **Fill Section 1 only**  
   Open `VERIFICATION_PLAN.md` and fill only **Section 1 (Project and DUT Overview)**. Replace the `<!-- TODO: ... -->` with real content (project name, DUT name, short description, interfaces). Use the "How to fill" hint under that section in the template, or see [FILL_GUIDES.md](FILL_GUIDES.md).

3. **Run the script**  
   From the repo root run: `./scripts/module1.sh`. It will report missing files, remaining TODOs, and checklist status. Fix any missing files first; then reduce TODOs section by section.

4. **Add the next section and repeat**  
   Fill Section 2 (Verification Objectives and Scope), run `./scripts/module1.sh` again, then Section 3, and so on. Use the script output "How to fix" lines to know what to do next.

5. **Do the same for REQUIREMENTS_MATRIX.md and CHECKLIST.md**  
   Populate the requirements matrix (at least the Requirement List §2 with rows like R1, R2, …). Check off checklist items as you complete them (`- [ ]` → `- [x]`).

6. **Use the reference when stuck**  
   Open `module1/.solutions/` to see a full example (e.g. stream_fifo). Compare section by section; the scripts do not grade "correctness," only structure and completeness.
   See [USE_CASES.md](USE_CASES.md) for the Stream FIFO use case and how to apply templates to other DUTs (e.g. UART).

This way you learn by doing, with fast feedback from the validation script after each step.

---

## 5. What the Scripts Check (and What They Don't)

### 5.1 What scripts can check well
...
### 5.2 What scripts cannot fully check
